...
-- Update the unique constraint to include variant_id
...
-- Add addresses table if missing
...
CREATE POLICY "addresses_user_own" ON public.addresses FOR ALL USING (auth.uid() = user_id);
CREATE POLICY "addresses_admin_all" ON public.addresses FOR ALL USING (
  EXISTS (
    SELECT 1 FROM public.profiles
    WHERE id = auth.uid() AND user_type = 'admin'
  )
